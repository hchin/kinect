...
public class DarthTracker extends Component
{
    ...
    private int fullmask = 255<<24;
    private BufferedImage bimg;
    int width, height;
    
    private final String SAMPLE_XML_FILE = "/home/hchin/kinect/Grip/ForceGrip/SamplesConfig.xml";
    
    //processing hack
	private PApplet P;
    public DarthTracker(PApplet parent)
    {
    	P =parent;
        		...
        		if (drawBackground || pixel != 0)
        		{
        			...
        			if (pixel != 0)
        			{
             		...
             		//imgbytes[3*pos+2] = (byte)(histValue*colors[colorID].getBlue());
        				int red = (int)(histValue*colors[colorID].getRed()) ;
        				int green = (int)(histValue*colors[colorID].getGreen());
                 		int blue = (int)(histValue*colors[colorID].getBlue());
                 		//System.out.println(pos+":"+red+":"+green+":"+blue);
                 		pixValue = red | (green << 8) | (blue << 16) | fullmask; 
                 		//System.out.print(pixValue+" ");
                 		P.pixels[pos] = pixValue;   
        			}
        		}
        	}
    ...
    void drawLine(HashMap<SkeletonJoint, SkeletonJointPosition> jointHash, SkeletonJoint joint1, SkeletonJoint joint2)
    {
		...
		if (jointHash.get(joint1).getConfidence() == 0 || jointHash.get(joint1).getConfidence() == 0)
			return;
		P.line((int)pos1.getX(), (int)pos1.getY(), (int)pos2.getX(), (int)pos2.getY());
    }
    
    public void drawSkeleton(int user) throws StatusException
    {
    	getJoints(user);
    	...
    	drawLine(dict, SkeletonJoint.LEFT_SHOULDER, SkeletonJoint.TORSO);
    	drawLine(dict, SkeletonJoint.RIGHT_SHOULDER, SkeletonJoint.TORSO);

    	drawLine(dict, SkeletonJoint.NECK, SkeletonJoint.LEFT_SHOULDER);
    	drawLine(dict, SkeletonJoint.LEFT_SHOULDER, SkeletonJoint.LEFT_ELBOW);
    	drawLine(dict, SkeletonJoint.LEFT_ELBOW, SkeletonJoint.LEFT_HAND);

    	drawLine(dict, SkeletonJoint.NECK, SkeletonJoint.RIGHT_SHOULDER);
    	drawLine(dict, SkeletonJoint.RIGHT_SHOULDER, SkeletonJoint.RIGHT_ELBOW);
    	drawLine(dict, SkeletonJoint.RIGHT_ELBOW, SkeletonJoint.RIGHT_HAND);

    	drawLine(dict, SkeletonJoint.LEFT_HIP, SkeletonJoint.TORSO);
    	drawLine(dict, SkeletonJoint.RIGHT_HIP, SkeletonJoint.TORSO);
    	drawLine(dict, SkeletonJoint.LEFT_HIP, SkeletonJoint.RIGHT_HIP);

    	drawLine(dict, SkeletonJoint.LEFT_HIP, SkeletonJoint.LEFT_KNEE);
    	drawLine(dict, SkeletonJoint.LEFT_KNEE, SkeletonJoint.LEFT_FOOT);

    	drawLine(dict, SkeletonJoint.RIGHT_HIP, SkeletonJoint.RIGHT_KNEE);
    	drawLine(dict, SkeletonJoint.RIGHT_KNEE, SkeletonJoint.RIGHT_FOOT);
        ...
        try
		{
			...
			for (int i = 0; i < users.length; ++i)
			{
				...
				if (printID)
				{
					...
					P.text(label, (int)com.getX(), (int)com.getY());
				}
			}
		} catch (StatusException e)
		{
